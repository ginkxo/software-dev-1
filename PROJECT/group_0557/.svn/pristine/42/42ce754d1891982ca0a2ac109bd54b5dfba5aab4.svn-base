...
public class Shopper extends User {
	...
	public Shopper(String userName, String password, City city, String address, int userID) {
		this.userName = userName;
		this.password = password;
		invoices = new HashMap<Integer, Invoice>();
		cart = new HashMap<Product, Integer>();		
	...
	/**
	 * @param ID
	 * @return The invoice associated with invoiceID ID
	 */
	...
	/**
	 * @return HashMap<Invoice ID: Invoice object>
	 */
	public HashMap<Integer, Invoice> getAllInvoices() {
		return invoices;
	}
	 ...
	 * @throws IOException 
	 */
	public boolean addToCartandUpdate(Product product, int quantity) throws IOException{
	...
	public int buyItNow(Product product, int quantity) throws IOException {
		if (quantity > 0 && quantity <= product.getQuantity() && city != null && address != null && Store.graph.getBestDistributionCenter(city) != null)
		{
	 ...
	 * @return -> true is update successful, false otherwise.
	 * @throws IOException
	 */
	public boolean modifyCart(Product product, int quantity) throws IOException {
		if (quantity < 0) return false;
		if (!this.getCart().keySet().contains(product)) {
			return addToCartandUpdate(product, quantity);
		}
		int amountInCart = this.getCart().get(product);
		if (quantity < amountInCart) {
			int amountToRemove = amountInCart - quantity;
			removeFromCart(product, amountToRemove);
			return true;
		}
		else if (quantity > amountInCart) {
			int amountToAdd = quantity - amountInCart;
			return addToCartandUpdate(product, amountToAdd);
		}
		return false;
	}
	/**
